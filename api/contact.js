...
import { checkRateLimit, getAllowedOrigin, validateEmail, sanitizeString } from './_lib/auth.js';
...
export default async function handler(req) {
  ...
  if (!rl.allowed) return new Response(JSON.stringify({ error: 'Trop de messages envoyés. Réessayez dans 1 heure.' }), { status: 429, headers: H });
  ...
  if (resendKey) {
    const subjectLabel = SUBJECTS[subject] || 'Autre';
    await fetch('https://api.resend.com/emails', {
      ...
      body: JSON.stringify({
        ...
        html: `<div style="font-family:sans-serif;max-width:600px;margin:0 auto"><h2 style="color:#1e293b">Nouveau message — Emploia</h2><table style="width:100%;border-collapse:collapse"><tr><td style="padding:8px;color:#64748b;font-weight:600;width:120px">De</td><td style="padding:8px">${name} &lt;${email}&gt;</td></tr><tr style="background:#f8fafc"><td style="padding:8px;color:#64748b;font-weight:600">Sujet</td><td style="padding:8px">${subjectLabel}</td></tr><tr><td style="padding:8px;color:#64748b;font-weight:600">Message</td><td style="padding:8px;white-space:pre-line">${message}</td></tr></table><p style="color:#94a3b8;font-size:12px;margin-top:20px">Répondre directement à cet email pour contacter ${name}.</p></div>`,
      }),
    });
